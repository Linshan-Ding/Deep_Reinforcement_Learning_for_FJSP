...
class Instance():
    ...
    @property
    def p_rjm(self):
        return round(uniform(5, 10), 1)
    """机器的待机功率"""
    @property
    def p_m_idle(self):
        return round(uniform(0.5, 0.9), 1)
    ...
    def process_information(self):
        ...
        machine_rj_dict = {r: {j: tuple(np.random.choice(self.machine_tuple, 4, replace=False))
        ...
        time_gap_s_dict = {s: sum([time_rj_dict[r][j] * count_sr_dict[s][r] for r in self.kind_tuple for j in task_r_dict[r]]) * self.DDT/(self.machine_count*2)
    ...
    def write_file(self):
        ...
        file_csv = {'based_data.csv': ['kind_count', 'machine_count', 'order_count', 'DDT'],
                    ...
                    'machine_data.csv': ['machine', 'idle_power'],
        ...
        for csv_name, header in file_csv.items():
            ...
            with open(data_file, 'w', newline='') as f:
                ...
                if csv_name == 'based_data.csv':
                    ...
                    for m, power in self.power_m_dict.items():
                        rows.append([m, power])
                else:
