...
class MO_FJSSP_Environment(SO_FJSSP_Environment):
    ...
    def __init__(self, use_instance=True, **kwargs):
        ...
        self.actions = tuple([(task_rule, machine_rule) for task_rule in range(6) for machine_rule in range(3)])
    ...
    def static_state_extract(self):
        ...
        return np.array([DDT, M, R, N_ave, N_std, J_ave, J_std])
    ...
    def step(self, action, weight_vector=None, completion=None, tardiness=None):
        ...
        # print("双重动作", actions)
        task_rule = actions[0] + 1  # 工序类型选择规则
        machine_rule = actions[1] + 1  # 机器选择规则
...
if __name__ == '__main__':
    ...
    while not env_object.done:
        ...
        action = env_object.actions.index((3, 1))
